...
computational cost.
We validate the periodic and spin-polarized implementation	
against \texttt{Octopus} for
one-dimensional H$_2$ chains, two-dimensional hexagonal BN,
three-dimensional Si,
and spin-polarized \ch{CrI3}, comparing
principal spectral features and relative optical response.
As a large-scale scientific application, we investigate negatively
charged nitrogen-vacancy (NV$^-$) centers in diamond,
a prototypical point defect system of
substantial interest for quantum sensing and quantum information
applications.\cite{NV-Doherty-2013} 
These calculations examine polarization selection rules,
spin-channel selectivity, and active-space dependence, followed
by a study of defect concentration and collinear magnetic
configurations in a common 4096-site diamond framework.
The largest single-defect calculation contains 4095 atoms and
one vacancy, demonstrating the capacity of the method to
investigate dilute defects in large periodic supercells.
...
pseudopotential contributions, active-space  density matrix propagation,
the collinear spin extension, and
the extraction of optical response functions. 
Section~\ref{sec:implementation}
describes the numerical implementation in
\texttt{RMG}, including the applied perturbation, current
evaluation, propagation, parallel organization, and spectral
processing.
Section~\ref{sec:benchmarks} presents the validation benchmarks.
Section~\ref{sec:nv-centers} presents the NV$^-$-center application, including
optical selection rules, active-space dependence, defect
concentration effects, and comparisons of collinear magnetic
configurations.
Section~\ref{sec:summary} concludes with a Summary and Outlook.
