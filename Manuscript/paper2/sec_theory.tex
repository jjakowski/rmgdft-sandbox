...
\myblue{[Brief overview of RMG: real-space grid, multigrid acceleration,
...
\myblue{[Note from CONTEXT.md: the code uses norm-conserving PP only
...
\myblue{[DERIVATION NEEDED from Jacek's LaTeX files.
Content outline:
...
Hamiltonian.
\myblue{[Expand: write out $\hat{H}_{\mathrm{KS}}(t)$ with all terms
including the EM coupling, then derive the long-wavelength limit.]}

==============================


We begin from the minimal-coupling Hamiltonian for electrons interacting with
an external electromagnetic field. In the Coulomb gauge, $\nabla\cdot\vb A=0$,
...
\frac{1}{2m}
\left(
\hat{\vb p}_i+\frac{e}{c}\vb A(\vb r_i,t)
\right)^2
-e\,\Phi(\vb r_i,t)
\right]
+\hat{W}_{ee}
+\hat{V}_{\mathrm{ion}},
\label{eq:minimal_coupling_manybody}
\end{equation}
where $m$ is the electron mass, $-e$ is the electron charge,
$\hat{\vb p}_i=-\imath\hbar\nabla_i$ is the canonical momentum operator,
$\vb A(\vb r,t)$ and $\Phi(\vb r,t)$ are the vector and scalar potentials,
respectively, $\hat{W}_{ee}$ is the electron--electron interaction, and
$\hat{V}_{\mathrm{ion}}$ denotes the electron--ion potential.
The corresponding time-dependent Kohn--Sham Hamiltonian inherits the same
minimal-coupling structure in its one-particle part.

For a monochromatic external field, the vector potential may be represented as
a plane wave,
\begin{equation}
\vb A(\vb r,t)=
\vb A_0
e^{\imath\omega t-\imath\vb q\cdot\vb r}
+\vb A_0^{\,*}
e^{-\imath\omega t+\imath\vb q\cdot\vb r},
\label{eq:plane_wave_A}
\end{equation}
where $\omega$ is the angular frequency, $\vb q$ is the wave vector, and
$\vb A_0$ is the complex amplitude. The associated electric field is
\begin{equation}
\vb E(\vb r,t)=
-\nabla\Phi(\vb r,t)
-\frac{1}{c}\frac{\partial \vb A(\vb r,t)}{\partial t}.
\label{eq:E_from_potentials}
\end{equation}

For optical excitations of finite systems and for crystalline unit cells or
supercells whose spatial extent is much smaller than the wavelength of the
external field, the phase factor in Eq.~\eqref{eq:plane_wave_A} varies only
weakly over the electronic region of interest. In that case,
\begin{equation}
|\vb q\cdot\vb r|\ll 1,
\label{eq:dipole_condition}
\end{equation}
and the exponential may be expanded as
\begin{equation}
e^{-\imath\vb q\cdot\vb r}
=
1
-\imath\vb q\cdot\vb r
+\mathcal{O}\!\left[(\vb q\cdot\vb r)^2\right].
\label{eq:plane_wave_expansion}
\end{equation}
Retaining only the leading term yields the long-wavelength, or dipole,
approximation,
\begin{equation}
\vb A(\vb r,t)\approx \vb A(t),
\label{eq:uniform_A}
\end{equation}
that is, the vector potential becomes spatially uniform over the simulation
cell. In this limit, the electric field is likewise spatially uniform to
leading order and is given by
\begin{equation}
\vb E(t)=
-\frac{1}{c}\frac{d\vb A(t)}{dt},
\label{eq:E_uniform_from_A}
\end{equation}
provided the scalar potential is chosen to vanish.

Equation~\eqref{eq:uniform_A} is the key simplification that leads to two
natural and gauge-equivalent forms of the light--matter interaction. In the
\emph{velocity gauge}, one sets
\begin{equation}
\Phi(\vb r,t)=0,
\qquad
\vb A(\vb r,t)\rightarrow \vb A(t),
\label{eq:velocity_gauge_choice}
\end{equation}
so that the one-particle Hamiltonian becomes
\begin{equation}
\hat{h}^{\mathrm{VG}}(t)=
\frac{1}{2m}
\left(
\hat{\vb p}+\frac{e}{c}\vb A(t)
\right)^2
+\hat{v}_{\mathrm{KS}}(t),
\label{eq:H_velocity_gauge}
\end{equation}
where $\hat{v}_{\mathrm{KS}}(t)$ collects the local and nonlocal ionic terms,
the Hartree potential, and the exchange--correlation potential.
The external field therefore couples through the momentum operator and, as
shown below, through the current density.

In the \emph{length gauge}, one instead sets
\begin{equation}
\vb A(\vb r,t)=0,
\qquad
\Phi(\vb r,t)=-\vb r\cdot\vb E(t),
\label{eq:length_gauge_choice}
\end{equation}
which gives
\begin{equation}
\hat{h}^{\mathrm{LG}}(t)=
...
+\hat{v}_{\mathrm{KS}}(t)
+e\,\vb r\cdot\vb E(t).
\label{eq:H_length_gauge}
\end{equation}
In this form, the external perturbation couples directly to the position
operator, or equivalently to the electric dipole operator.

Within the long-wavelength approximation, the velocity- and length-gauge
Hamiltonians are related by a time-dependent unitary transformation,
\begin{equation}
\psi^{\mathrm{LG}}(\vb r,t)=
\exp\!\left[
\frac{\imath e}{\hbar c}\,
\vb A(t)\cdot\vb r
\right]
\psi^{\mathrm{VG}}(\vb r,t).
\label{eq:VG_to_LG_unitary}
\end{equation}
Thus, the two gauges describe the same physics, but they emphasize different
observables. The length gauge is naturally expressed in terms of the dipole
moment, whereas the velocity gauge is naturally expressed in terms of the
momentum and current operators. This distinction becomes essential for periodic
systems, where the position operator is not a convenient observable under
periodic boundary conditions. For that reason, the periodic RT-TDDFT
formulation developed in this work is based on the velocity gauge.
...
\myblue{[Expand: write out all terms of $\tilde{V}^{\mathrm{nl}}$
...
\myblue{[Note: This subsection establishes the theoretical foundation
...
\myblue{[Expand into full derivation of current operator, paramagnetic
...
\end{equation}
\myblue{[The $k_\alpha$ term in eq.~\ref{eq:current-matrix} arises from
...
\myblue{[Adapt  from paper 1 (Jakowski et al. 2025, Section 2.2). ]}
...
\myblue{[Continue with Magnus expansion (refer to paper 1 eqs. and
cite), then commutator expansion for P(t+dt), self-consistency loop.
For complex H: add note about BCH-only path.]}

\myblue{[Note from code inspection (CONTEXT.md Gap 4): magnus() in the
...
\myblue{[Continue: derive spin-resolved current, define total and spin
...
\myblue{[Expand: relate $\sigma$ to $\varepsilon$; describe how three
...
\myblue{[Add: Berry phase derivation and density matrix form.
