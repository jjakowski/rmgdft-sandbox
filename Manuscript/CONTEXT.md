...
| Manuscript/source/ | Read-only: paper1_main.tex, derivation files, references.bib |
...
- Full gauge transformation of the nonlocal PP term (Mattiat Appendix B analog)
- Proof of equivalence between velocity gauge current and Berry phase polarization
- Any extended algebraic steps from the current operator derivation
- Possibly: spin-collinear extension of the BCH scheme

### Supplementary Information
Numerical validation details too long for main paper:
- Extended convergence tests (k-point, grid spacing, time step)
- Additional benchmark systems if needed
- Raw data tables
...
- Use \myred{} for any content that needs Jacek's review before finalizing.
...
| Spin handling | separate P_up,P_dn per spinpe | RmgTddft.cpp | rho.get_oppo(), compute_vxc |
...
## Identified gaps (from code inspection, session 1)

### Gap 1 -- i-factor sign consistency [PRIORITY]
