...
\newpage
%%%%%%%%%%%%%%%
\section { Classical Hamiltonian for a particle in Electromagnetic (EM) field}
\label{sec:gauge_classical} 

%We   show derivation of above  expression following  \emph{Minimal Coupling} article in  Wikipedia (\url{https://en.wikipedia.org/wiki/Minimal_coupling})

The Lagrangian of a particle with charge $q$  and mass $m$ in an electromagnetic field
is
\begin{align*}
...
{\bf are gauge invariant} and  {\bf are physically measurable}.
 ...
 The quantum  Hamitlonian for a particle in electromagnetic field  
\begin{align}\boxed{
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{ Gauge invariance of electric $\vb E$ and magnetic $\vb B$ fields}. 
...
%We now show how the  physical quantities  transform under gauge transformation.
The guiding principle is to choose is  that the gauge transformation does not change any physical  observable such us charge density and electric current, expectation value of position, momentum, energy.
...
Thus we have showns that the  gauge transformation  does not change magnetic field  ($\vb  B'=  \vb  B$)  nor   electric  field ($\vb  E'=  \vb  E$) 
...
%%%%%%%%%%%%%%%%%
\section{Quantum  Hamiltonian  for  a particle in  EM field }
...
We can see that the kinetic operator and scalar field  term  in  Vanderbilt Hamiltonian is the same  as  in Eq. \ref{eq:EM_gauge} assuminng  that  the particle is an electron with negative charge  $q=-e$.
...
\end{align*}
Using this gauge function $\chi(r,t)$ the  gauge transformed wave function and operator are  obtained as
\begin{align*}
    \Psi'(r,t) =& \exp{+\imath \frac{q}{\hbar c} \chi(r,t)} \Psi(r,t)= U(r,t) \Psi(r,t)\\
    O'         =&  \exp{+\imath \frac{q}{\hbar c} \chi(r,t)}  O  \exp{-\imath \frac{q}{\hbar c} \chi(r,t)} \Psi(r,t)=   U(r,t) \cdot O \cdot U^\dag(r,t)
\end{align*}
where $U(r,t) =\exp{+\imath \frac{q}{\hbar c}\chi(r,t)}$, and   $U(r,t)\cdot U^\dag (r,t)=I$, which shows that  that gauge transformation is unitary.
%(Notice: check the sign,  them  "minus" sign in from of A,E often comes from assumption  the we talk about electrons so  q=-1)     

We will now discuss and derive general rules for  gauge transformation in quantum mechanics. After that we   use these rules  to work transformation of our TDSE hamiltonian from length to velocity gauge

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{ General gauge transformation of TDSE }


We will now work  deriving/deducing general rules for gauge transformation of  wave function, canonicanl and  kinetic momentum operator. When in doubts we will use guiding  principles of the gauge transformation: that the physically measurable quantities cannot change.
We will verify that:  (1) Gauge transformed   kinetic/mechanical momentum operator has the same expectation  value   as the untransformed one. (2) We will verify that  momentum operator transformed  according to  Eq. \ref{eq:gauge_rule_Operator} leads to the same  expression as given by Eq. \ref{eq:gauge_rule_momentum_kinetical}. This will allow to use the same procedure to  be applied to kinetic energy operator  $\hat T$ and to the Hamiltonian.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Example 1: Expectation value of gauge transformed kinetic momentum.}
...
%------------------------------------------------------------------
\subsubsection{Example 2: Explicit transformation of kinetic momentum  }
...
%------------------------------------------------------------
\subsubsection{Example 3: Explicit transformation of time-derivative operator}
...
%\end{itemize}`
%------------------------------------------------------------
\subsubsection{Example 4: Time dependent Schroedinger equation}
...
\chi(r,t)  =& c \int_0^t \phi(r,t')  dt' \\
           ...
           =&   c \int_0^t (x E_x(t) +  y E_y(t) +z E_z(t)) dt' \\
            =& c \cdot \vec{\vb  r} \int_0^t \vec{\vb E}(r,t') dt' %\\
\end{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{ Other unfinished notes: Velocity gauge for periodic rt-TDDFT }
...
%%%%%%%%%%%%%%%
{\bf Maxwell+ Schroedinger  TDKS equaitions for periodic systems}


A comprehensive macroscopic Maxwell-TDDFT scheme is desctibed in  \cite{prb-2018-Yabana}. 
See also 2020 octopus paper \cite{octopus-rt-tddft-JCP-2020}.
